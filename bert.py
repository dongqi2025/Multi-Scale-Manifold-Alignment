...
# 多尺度BERT特征提取器
class MultiscaleBert(nn.Module):
    def __init__(self, model_name='bert-base-uncased'):
        super().__init__()
        local_path = "/ossfs/workspace/model/bert-base-uncased"

        self.bert = BertModel.from_pretrained(local_path, output_hidden_states=True)
    ...
    def forward(self, input_ids, attention_mask):
        ...
        def pool_layers(layer_indices):
            ...
            for i in layer_indices:
                # 步骤1：对每个token位置的特征进行平均 (seq_len维度池化)
                layer_pool = torch.mean(hidden_states[i], dim=1)  # 输出形状 [batch, 768]
...
if __name__ == "__main__":
    experiment_groups = {
        ...
        "only_geo_1": {
            ...
            "lambda_info": 0,
            "lambda_curv": 0,
            "save_dir": "./p2_bert/only_geo_1"
        },
    }
    base_config = {
        'bert_path': '/ossfs/workspace/model/bert-base-uncased',
    ...
    for group_name, group_config in experiment_groups.items():
        ...
        if embeddings_exist(save_dir):
            print(f"加载缓存数据: {group_name}")
            embeddings = load_embeddings(save_dir)
        else:
            print(f"开始训练: {group_name}")
